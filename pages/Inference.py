...
st.title("📉 Statistical Inference Tests")
...
if st.button("🚀 Run Test", type="primary"):
    ...
    if test_choice == "Price Distribution (Normality Test)":
        ...
        with col1:
            fig1 = px.histogram(df, x="price", nbins=50, title="Original Price Distribution",
                                labels={"price": "Price (LKR)"}, color_discrete_sequence=["#18DF89"])
            fig1.add_vline(x=df['price'].mean(), line_dash="dash", line_color="red", annotation_text="Mean")
            st.plotly_chart(fig1, use_container_width=True)
        with col2:
            fig2 = px.histogram(df, x="price_transformed", nbins=50, title="Log-Transformed Price Distribution",
                                labels={"price_transformed": "log(Price)"}, color_discrete_sequence=["#18DF89"])
            fig2.add_vline(x=df['price_transformed'].mean(), line_dash="dash", line_color="red", annotation_text="Mean")
        ...
        result_df = pd.DataFrame({"Statistic": [round(stat, 4)], "p-value": [round(p, 4)], "Decision": ["Reject H₀" if p < 0.05 else "Fail to reject H₀"]})
    ...
    # ====================== FUEL TYPE ======================
        ...
        if p < 0.05:
            st.success("✅ Reject H₀ → Median prices differ across fuel types")
        else:
            st.warning("❌ Fail to reject H₀ → No significant difference")
    ...
    # ====================== TRANSMISSION/GEAR ======================
        ...
        if p_val < 0.05:
            st.success("✅ Reject H₀ → Automatic vehicles have significantly higher prices")
        else:
            st.warning("❌ Fail to reject H₀ → No evidence Automatic > Manual")
        
        result_df = pd.DataFrame({"U Statistic": [u_stat], "p-value": [round(p_val, 4)], "Decision": ["Reject H₀" if p_val < 0.05 else "Fail to reject H₀"]})
        st.dataframe(result_df, use_container_width=True, hide_index=True)
    ...
    # ====================== ENGINE SEGMENT ======================
        ...
        if p < 0.05:
            st.success("✅ Reject H₀ → Significant difference across engine segments")
        else:
            st.warning("❌ Fail to reject H₀ → No significant difference")
        
        result_df = pd.DataFrame({"Statistic": [round(stat, 3)], "p-value": [round(p, 3)], "Decision": ["Reject H₀" if p < 0.05 else "Fail to reject H₀"]})
        st.dataframe(result_df, use_container_width=True, hide_index=True)
    ...
    # ====================== OTHER VARIABLES ======================
        ...
        if "Spearman" in test_choice:
            ...
            if p < 0.05:
                st.success("✅ Reject H₀ → Significant correlation")
            else:
                st.warning("❌ Fail to reject H₀ → No significant correlation")
            ...
            if p < 0.05:
                st.success("✅ Reject H₀ → Significant effect")
            else:
                st.warning("❌ Fail to reject H₀ → No significant effect")
            
            result_df = pd.DataFrame({"U Statistic": [round(u_stat, 1)], "p-value": [round(p, 4)], "Decision": ["Reject H₀" if p < 0.05 else "Fail to reject H₀"]})
            st.dataframe(result_df, use_container_width=True, hide_index=True)
            st.caption("**Interpretation**: This feature significantly affects vehicle price.")

st.divider()

# ── OVERALL ASSUMPTIONS & LIMITATIONS ────────────────────────────────
show_card(
